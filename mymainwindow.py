...
class my_ui(Ui_MainWindow):
    ...
    def serial_init(self):
        ...
        self.textEdit_5.setText("mode 1")
        ...
        self.pushButton_4.clicked.connect(self.pushButton4_slot)
        self.pushButton_5.clicked.connect(self.pushButton5_slot)
        self.pushButton_6.clicked.connect(self.pushButton6_slot)
        self.pushButton_7.clicked.connect(self.pushButton7_slot)
        self.pushButton.clicked.connect(self.pushButton_slot)
        self.pushButton_2.clicked.connect(self.pushButton2_slot)
        self.pushButton_11.clicked.connect(self.pushButton_11_slot)
        self.ser.readyRead.connect(self.readyall_slot)
        self.pushButton_9.clicked.connect(self.pushButton9_slot)
        self.pushButton_10.clicked.connect(self.pushButton10_slot)

        self.timer.timeout.connect(self.timer_slot)
        self.pushButton_8.clicked.connect(self.pushButton_8_slot)

    def pushButton10_slot(self):
        ...
        match np.round(rf_reg.predict(data2)[0]):
            case 1:
                self.label_10.setText('疲劳等级：清醒')
            case 2:
                self.label_10.setText('疲劳等级：轻度疲劳')
            case 3:
                self.label_10.setText('疲劳等级：重度疲劳')
    ...
    def readyall_slot(self):
        ...
        if self.radioButton.isChecked():
            self.serbuff = self.ser.readAll()[0]
            self.textEdit.insertPlainText(self.serbuff.hex() + '\n')  # str还可指定编码格式
            self.series.append(self.x,float(int.from_bytes(self.serbuff, byteorder='big')))
            self.textEdit.moveCursor(QTextCursor.MoveOperation.End)
            self.x += 1
        if self.radioButton_2.isChecked():
            self.serbuff = self.ser.readAll()
            self.textEdit.insertPlainText(str(self.serbuff, encoding='utf-8', errors='ignore'))
            try:
                self.series.append(self.x,float(str(self.serbuff, encoding='utf-8', errors='ignore')))
                self.x += 1
    ...
    def chart_init(self):
        ...
        self.chart1.axisY().setRange(-100, 200)  # 设置y1轴范围
...
########################################################################################################
        ...
        self.chart2.axisX().setRange(0, 2000)
        self.chart2.axisY().setTitleText('数值')
        self.chart2.legend().setVisible(False)
        self.chart2.axisY().setRange(-100, 200)  # 设置y1轴范围
        self.ChartView_2.setChart(self.chart2)
        self.chart1.zoom(0.99999)

    def pushButton9_slot(self):
        wd, m = hp.process(hp.scale_data(self.ppg_filt_data), self.sample_rate)
        ...
        time_domain_features = ha.get_time_domain_features(rr)
        ...
        # min_hr       最小心率
        # std_hr       心率标准差
        frequency_domain_features = ha.get_frequency_domain_features(rr)
        # 返回字典，包含如下键值：
        # total_power   总功率密度谱
        ...
        # lfnu          归一化低频功率
        # hfnu          归一化高频功率
        poincare_plot_features = ha.get_poincare_plot_features(rr)
        # 返回如下键值：
        ...
        with open('hrv1.csv', 'a', newline='') as csvfile:
            writer = csv.DictWriter(csvfile, fieldnames=(
                        list(time_domain_features) + list(frequency_domain_features) + list(poincare_plot_features)))
            writer.writeheader()
        ...
        with open('hrv2.csv', 'w', newline='') as csvfile:
            ...
            writer.writeheader()
            writer.writerow({**m})
    def pushButton4_slot(self):
        self.textEdit.clear()
        self.x = 0.0
        self.series.clear()
    def pushButton_11_slot(self):
        print(self.textEdit_5.toPlainText().encode()+b'\r')
        self.ser.write(self.textEdit_5.toPlainText().encode()+b'\r\n')

    def pushButton5_slot(self):

        textFile = QFileDialog(None).getOpenFileName(None, '读取数据', '', 'TEXT (*txt)')
        if textFile[0]:
            self.x = 0.0
            self.series.clear()
    ...
    def pushButton6_slot(self):
        textFile = QFileDialog(None).getSaveFileName(None, '保存数据', '', 'TEXT (*txt)')
        if textFile[0]:
            with open(textFile[0], 'w') as save:
                save.write(self.textEdit.toPlainText())

    def pushButton7_slot(self):
        imagefile = QFileDialog(None).getSaveFileName(None, '保存图片', '', 'PNG (*png)')
        if imagefile[0]:
    ...
    def pushButton2_slot(self):

        self.sample_rate = int(self.lineEdit.text())
        self.textEdit_2.clear()
        self.x2 = 0.0
        self.series2.clear()

        ppg_data = [point.y() for point in self.series.pointsVector()]

        # 带通滤波
        [b, a]  = signal.butter(int(self.comboBox_6.currentText()), [(2 * self.doubleSpinBox_2.value()) / self.sample_rate,
                                                                  (2 * self.doubleSpinBox.value()) / self.sample_rate],
                             'bandpass')  # 最大只能设置为4阶，scipy函数限制
        self.ppg_filt_data = signal.filtfilt(b, a, ppg_data)  # 为要过滤的信号
        for point in self.ppg_filt_data:
    ...
    def timer_slot(self):
        ...
        if self.port_list != [port.portName() for port in port_list]:
            self.port_list = [port.portName() for port in port_list]
            self.comboBox.clear()
